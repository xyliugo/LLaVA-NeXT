...
#SBATCH --output=logs/llava-next-pretrain.out
...
BASE_RUN_NAME="llava-next-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-pretrain_blip558k_plain"
...
srun --ntasks=1 --cpus-per-task=16 --gpus=2 \
    ...
    --bf16 True \
    --output_dir ./checkpoints/llava-next-qwen2.5-1.5b-clip-pretrain \
    --num_train_epochs 1 \
